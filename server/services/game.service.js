...
export function addScore(teamId, points) {
  const t = team(teamId);
  if (!t) return;
  const previous = Number(t.score) || 0;
  const next = Math.max(0, previous + Number(points || 0));
  t.score = next;
  if (next !== previous) {
...
export function showQuestion(opts = {}) {
  ...
  if (game.round === "ve_dich") {
    ...
    game.veDich.stealOpen = false; // reset cửa sổ cướp khi bắt đầu câu hỏi mới
    game.veDich.stealPending = null;
    const answerSeconds = vedich.getAnswerSeconds(game);
    console.log(`[vedich] showQuestion: stealOpen reset to false, timer ready at ${answerSeconds}s`);
    const star =
      game.veDich.starQuestion === (game.veDich.pickIndex ?? 0)
        ? " • Ngôi sao hy vọng"
        : "";
    game.display.note = `${team(game.currentTeam)?.name || ""} • ${q?.points || 20} điểm${star}`;
    // Nạp sẵn thời lượng theo điểm nhưng chưa chạy để MC có thời gian đọc câu hỏi.
    // MC bấm "Bắt đầu tính giờ" mới bắt đầu đếm cho thí sinh.
    setTimer(answerSeconds, false);
    clearVedichAuto();
  }
  saveDb();
  emit();
...
export function markAnswer(correct, teamId = null) {
  ...
  if (game.round === "ve_dich") {
    ...
    ved.stealOpen = true;
    game.buzzer = { open: true, locked: false, winner: null, order: [], blocked: [tid] };
    pauseTimer();
    saveDb();
    emit();
    return;
...
// MC bấm "Hiện đáp án" sau khi hết giờ và bấm Sai — lật đáp án để khán giả thấy.
// Không thay đổi trạng thái steal (cửa sổ cướp vẫn mở nếu đang mở).
export function vedichRevealAnswer() {
  const game = g();
  if (game.round !== "ve_dich") {
  ...
  if (game.display.mode !== "question") {
    const err = new Error("Chưa hiện câu hỏi.");
    err.status = 400;
    throw err;
  }
  game.display.answerRevealed = true;
...
export function resetBuzzer(open = false) {
  const game = g();
  game.buzzer = { open, locked: false, winner: null, order: [], blocked: [] };
  saveDb();
  emit();
}
